...
def get_preds_fromhm(hm, center=None, scale=None):
	max = np.max(hm.reshape(hm.shape[0], hm.shape[1], hm.shape[2] * hm.shape[3]), 2)
	idx = np.argmax(hm.reshape(hm.shape[0], hm.shape[1], hm.shape[2] * hm.shape[3]), 2)
	idx += 1
	preds = np.tile(idx.reshape(idx.shape[0], idx.shape[1], 1),[1, 1, 2]).astype('float')
	lm1 = lambda x: (x - 1) % hm.shape[3] + 1
	preds[..., 0] = lm1(preds[..., 0])
	preds[...,1] = preds[..., 1] -1
	preds[...,1] =np.floor((preds[...,1]/hm.shape[2]))+1
